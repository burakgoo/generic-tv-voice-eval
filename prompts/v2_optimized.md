...
* **TV__SWITCH_CHANNEL** — Explicitly switch/tune to a specific live TV channel (by name or number).
* **TV__NAVIGATE** — Navigation by channel name when tuning is not **explicit** (e.g. a command that could be used to navigate or scroll within a channel list).
...
* For `canonical`:
  ...
  * `channel`: map the user's phrase to the closest official channel name and use it verbatim as the canonical value (including case/spacing). Match tolerantly — ignore case, spacing, and punctuation (dots, hyphens), and treat `ß`↔`ss`, `ä`↔`ae`, `ö`↔`oe`, `ü`↔`ue` as equivalent (e.g. *n tv* → `n-tv`). For a bare channel number, keep the number. Do NOT attempt to guess the canonical channel/app name (e.g. adding 'HD'). Map to the EXACT literal text value you hear. **Whenever a channel intent (TV__SWITCH_CHANNEL or TV__NAVIGATE) is chosen, a `channel` entity must always be extracted** — never return such an intent with no `channel`. Also extract `channel` in zapping, EPG, or search context when a channel is referenced.
  * `app`: map the user's phrase to the closest official app name and use it verbatim as the canonical value. Do NOT attempt to guess the canonical channel/app name (e.g. adding 'HD'). Map to the EXACT literal text value you hear. Extract `app` whenever a streaming app is referenced (open app or search context). **Whenever an app intent (TV__OPEN_PARTNER_APP) is chosen, an `app` entity must always be extracted** — never return such an intent without an `app`.
...
* **TV__SWITCH_CHANNEL**: Use **only** for explicit tuning/switching/opening (“switch to”, “tune to”, “open channel 2”) to a **specific channel**. The intent is to **watch the mentioned channel now**. If it's not clear if the channel or an app was meant, prefer **TV__SWITCH_CHANNEL**.
* **TV__NAVIGATE**: Use for channel-related requests that are **not clearly** a switch (e.g., “go to CNN,” “to MTV”, “jump to BBC”) and could also be used for scrolling inside channel lists.
...
Output:
```json
{"utterance": "Switch to channel 7", "intent": "TV__SWITCH_CHANNEL", "entities": [{"name": "channel", "literal": "Channel 7", "canonical": "7"}]}
```
...
Output:
```json
{"utterance": "Cymbal go to ORF 2", "intent": "TV__NAVIGATE", "entities": [{"name": "channel", "literal": "ORF 2", "canonical": "ORF2_HD"}]}
```
...
**Example 11**  
Input: `Show me the program`
Output:
```json
{"utterance": "Show me the program", "intent": "TV__OPEN_MENU", "entities": [{"name": "menu_type", "literal": "Program", "canonical": "MI_epg"}]}
```
...
Output:
```json
{"utterance": "To channel 11", "intent": "TV__NAVIGATE", "entities": [{"name": "channel", "literal": "Channel 11", "canonical": "11"}]}
```
...
Output:
```json
{"utterance": "Go back", "intent": "TV__RETURN"}
```

**Example 14**  
Input: `Start teletext`
Output:
```json
{"utterance": "Start teletext", "intent": "TV__OPEN_PARTNER_APP", "entities": [{"name": "app", "literal": "Teletext", "canonical": "Teletext"}]}
```
